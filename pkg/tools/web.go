...
func (t *WebFetchTool) fetchURL(ctx context.Context, urlStr string) (*fetchResult, *ToolResult) {
	...
	if resp.StatusCode == http.StatusForbidden && resp.Header.Get("Cf-Mitigated") == "challenge" {
		...
		if err2 == nil {
			...
			return nil, ErrorResult(err2.Error())
		}
	}
...
func (t *WebFetchTool) fetchAndConvert(
	...
	case mediaType == "text/html" || looksLikeHTML(bodyStr):
		text, extractor = htmlConv(bodyStr, fr)
...
func NewWebFetchMarkdownTool(maxChars int, fetchLimitBytes int64) (*WebFetchMarkdownTool, error) {
	return NewWebFetchMarkdownToolWithProxy(maxChars, "", fetchLimitBytes, nil)
}
...
func NewWebFetchMarkdownToolWithProxy(
	...
	proxy string,
	fetchLimitBytes int64,
	privateHostWhitelistEntries []string,
) (*WebFetchMarkdownTool, error) {
	base, err := NewWebFetchToolWithConfig(maxChars, proxy, "", fetchLimitBytes, privateHostWhitelistEntries)
	if err != nil {
		return nil, err
	}
